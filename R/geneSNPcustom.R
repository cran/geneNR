...
#'
#' @param data_file The input data in .csv format. (sample_data_wheat_custom for demo purpose)
...
geneSNPcustom <- function(data_file,  crop = "wheat") {
  ...
  sample_data_wheat_custom <- sample_data_wheat_custom


  # Load data
  if (data_file == "sample_data_wheat_custom") {
    data <- sample_data_wheat_custom
  }
    else {
  ...
  required_columns <- c("start","stop", "Chr", "traits", "SNP")
