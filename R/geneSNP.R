...
#'
#' @param data_file The input data in .csv format. (sample_data_wheat or sample_data_rice for demo purpose)
#' @param upstream The search window upstream of the current position of the SNP. (default: 1000000)
#' @param downstream The search window downstream of the current position of the SNP. (default: 1000000)
#' @param crop Either "wheat" or "rice". (default: wheat)
#'
#' @returns A data frame containing traits, SNP, gene_id, gene_size, and gene_type.
#' @export
#'
#' @examples
...
geneSNP <- function(data_file, upstream = 1000000, downstream = 1000000, crop = "wheat") {
  ...
  sample_data_wheat <- sample_data_wheat
  sample_data_rice <- sample_data_rice

  # Load data
  if (data_file == "sample_data_wheat") {
    data <- sample_data_wheat
  } else if (data_file == "sample_data_rice") {
    data <- sample_data_rice
  } else {
  ...
  required_columns <- c("Pos", "Chr", "traits", "SNP")
  ...
  data$start <- data$Pos - upstream
  data$stop <- data$Pos + downstream
  data$search_window <- paste0(data$Chr, ":", data$start, "-", data$stop)
  data$url <- stringr::str_replace(base_url, "'a'", data$search_window)
  data$file_name <- paste0(data$traits, "_", data$SNP, "_", data$Chr, "_", data$start, "_", data$stop)
  ...
  split_columns <- stringr::str_match(filtered_df$file_name, "^(.*?)_(A[^_]+)_(.*)\\.txt$")
  filtered_df$traits <- split_columns[, 2]
  filtered_df$SNP <- split_columns[, 3]
  filtered_df$search_window <- split_columns[, 4]

  filtered_df <- filtered_df[, c("traits", "SNP", "search_window", "gene_size", "gene_id", "gene_type")]
